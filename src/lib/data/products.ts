...
export const getProductFashionDataByHandle = async function (handle: string) {
  // Skip fashion data fetch for test products or incomplete products
  if (!handle || handle === 'test' || handle.startsWith('test-')) {
    console.log(`Skipping fashion data fetch for test product: ${handle}`)
    return {
      materials: []
    }
  }
  ...
  try {
    ...
    console.error(`Failed to fetch fashion data for product ${handle}:`, error)
    // Return empty materials array as fallback
    return {
      materials: []
    }
  }
